...
//
// 본 예제는 6_GP_Tactile4_Driverlib.c와 완전히 동일한 DriverLib GPIO 제어 코드를 쓰되,
...
// Bit-Field 방식의 6_GP_Tactile4_Bitfield.c와 함께 세 가지 방식을 비교해 보실 수 있습니다.
//
//************************************************************************************************************************************************************************
...
void TactSwitch_Task(void *pvParameters);
...
//	   (Idle Loop를 직접 돌리지 않고, TactSwitch_Task( )를 스케줄러에 맡깁니다 — 아래 10번 참고)
	xTaskCreateStatic(TactSwitch_Task,			// 태스크 함수
	                  "TactSwitch Task",		// 이름(디버깅용)
	                  STACK_SIZE,				// 스택 크기(워드)
	                  NULL,						// 파라미터 없음
	                  tskIDLE_PRIORITY + 1,
	                  switchTaskStack,
...
//
// TactSwitch_Task - Idle(Background) Loop에 해당하는 실제 동작. DriverLib 버전의 메인
// 루프와 완전히 같은 로직이며, DEVICE_DELAY_US busy-wait 대신 vTaskDelay로 다른
...
void TactSwitch_Task(void *pvParameters)
{
    ...
    for(;;)
    {
        ...
        vTaskDelay(10 / portTICK_PERIOD_MS);	// 10msec 지연 (폴링 주기)
    }
}

//
// initSwitchGpio - GPIO67~GPIO70을 전부 입력(내부 풀업, 동기 입력)으로 설정. DriverLib
// 버전(6_GP_Tactile4_Driverlib.c)과 설정 코드를 완전히 동일하게 맞춰서, 두 예제의 차이가
