# 택트 스위치 4개 테스트 — FreeRTOS 버전 — TMS320F28P659DK8-Q1

[6_GP_Tactile4_Driverlib](../6_GP_Tactile4_Driverlib/README.md)(DriverLib 버전)와 **GPIO 제어 코드는
완전히 동일**하게 두고, "메인 루프에서 직접 딜레이"를 "FreeRTOS 태스크 1개가 스케줄러
위에서 도는 방식"으로만 바꾼 버전입니다.
...
- 태스크는 스위치 태스크 1개 + FreeRTOS가 항상 만드는 Idle 태스크 1개, 소프트웨어
  타이머·큐는 안 씁니다.
- `DEVICE_DELAY_US` busy-wait 대신 `vTaskDelay(10 / portTICK_PERIOD_MS)`로 10msec마다
...
GPIO 입력 설정, Active High 판정, 에지 검출로 `pressCount`를 세는 방식, 그리고
"한 번 눌렀는데 왜 여러 번 카운트될 수 있는지"(스위치 바운스)는 DriverLib
버전과 완전히 동일합니다 — 자세한 설명은
[6_GP_Tactile4_Driverlib의 "동작 원리"](../6_GP_Tactile4_Driverlib/README.md#동작-원리)를
참고하세요.

이 버전에서 유일하게 다른 점은 그 로직이 **FreeRTOS 태스크 안에서** 돈다는
것뿐입니다. `TactSwitch_Task()`의 `for(;;)` 루프 내부(GPIO 읽기 → 에지 검출 →
`pressCount` 갱신)는 DriverLib 버전의 메인 루프와 한 글자도 다르지 않고, 다만
`DEVICE_DELAY_US(10000)`(그동안 CPU를 그냥 붙잡고 기다림) 대신 `vTaskDelay(10 /
portTICK_PERIOD_MS)`(그동안 CPU를 Idle 태스크에게 넘겨줌)를 씁니다. 즉 "스위치를
어떻게 읽고 판단하는가"는 RTOS를 얹어도 전혀 바뀌지 않습니다 — RTOS는 어디까지나
"언제 이 코드를 실행할 차례인가"만 결정합니다.

## Import → Build → Flash → Run
1. CCS에서 `CCS/6_GP_Tactile4_Freertos.projectspec`를 Import
...
`DEVICE_DELAY_US` busy-wait와 미세하게 다를 수 있습니다. `tactSwitchState`/`pressCount`는
...
| **FreeRTOS(이 폴더)** | 6_GP_Tactile4_Freertos | DriverLib + 태스크 스케줄링, 정적 할당 |
...
|---|---:|---:|---:|---:|
| **FreeRTOS(이 폴더)** | 5,084 B | 820 B | 1,239 B (RTS 스택 512B + 태스크 정적 스택 512B + 전역변수 215B) | **7,143 B** |
...
비트필드 버전(rw data에 주변장치 레지스터 구조체 심볼이 잡혀서 부풀어 보이는 5,964B)과
...
- 게시판 글: https://tms320f28x.co.kr/board/view.php?bdId=tms320f28xevmv2&sno=105
